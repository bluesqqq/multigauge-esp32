...
void TestUnitType() {
    ...
    TEST_ASSERT_EQUAL_STRING("Base", units[0].name.c_str());
    TEST_ASSERT_EQUAL_STRING("Second", units[1].name.c_str());
    TEST_ASSERT_EQUAL_STRING("Third", units[2].name.c_str());

    TEST_ASSERT_EQUAL_STRING("Base", testUnitType.getBaseUnit().name.c_str());

    TEST_ASSERT_EQUAL_STRING(testUnitType.getUnit(-1).name.c_str(), testUnitType.getDefaultUnit().name.c_str());
    TEST_ASSERT_EQUAL_STRING(testUnitType.getUnit(3).name.c_str(), testUnitType.getDefaultUnit().name.c_str());

    TEST_ASSERT_EQUAL_STRING("Base", testUnitType.getDefaultUnit().name.c_str());

    testUnitType.setDefaultUnit(1);
    TEST_ASSERT_EQUAL_STRING("Second", testUnitType.getDefaultUnit().name.c_str());
    testUnitType.setDefaultUnit(2);
    TEST_ASSERT_EQUAL_STRING("Third", testUnitType.getDefaultUnit().name.c_str());
...
void TestConversions() {
    ...
    TEST_ASSERT_EQUAL_FLOAT(212, temperatureUnitType.convert(100, 0, 1)); // 100C to F
    TEST_ASSERT_EQUAL_FLOAT(373.15, temperatureUnitType.convert(100, 0, 2)); // 100C to K
    TEST_ASSERT_EQUAL_FLOAT(32, temperatureUnitType.convert(0, 0, 1)); // 0C to F
}
